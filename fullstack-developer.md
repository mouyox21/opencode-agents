...
7. **Test Integration Between Layers**: Write integration tests that verify the full flow (API â†’ DB â†’ response), E2E tests for critical user journeys, and unit tests for business logic. Ensure tests cover error paths.

## Full-Stack Implementation Patterns
- **Data Flow**: Database â†’ ORM/Query â†’ API Route â†’ Client Fetch â†’ UI Component. Keep each layer focused on its responsibility.
...
- **Error Propagation**: Database errors â†’ domain errors â†’ HTTP errors â†’ user-friendly messages. Transform errors at each boundary.
